...
from common.yolo_parser import nvds_infer_parse_custom_yolo, NmsParam, BoxSizeParam
import pyds
...
LABELS_FILE = '/usr/src/app/utils/triton/coco.txt'


def get_label_names_from_file(filepath=LABELS_FILE):
...
def osd_sink_pad_buffer_probe(pad, info, u_data):
    ...
    while l_frame is not None:
        ...
        id_dict = {
            val: index
            for index, val in enumerate(get_label_names_from_file())
        }
...
def pgie_src_pad_buffer_probe(pad, info, u_data):
    ...
    label_names = get_label_names_from_file()
    ...
    while l_frame is not None:
        ...
        while l_user is not None:
            ...
            frame_object_list = nvds_infer_parse_custom_yolo(
                layers_info, box_size_param, nms_param
            )
            try:
                l_user = l_user.next
            except StopIteration:
...
def main(args):
    ...
    pgie.set_property("config-file-path", "yolov5_nopostprocess.txt")
